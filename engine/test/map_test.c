...
{
    map_int_int map = map_int_int_new(test_heap_alloc);
...
{
    map_int_int map = map_int_int_new(test_heap_alloc);
    TEST_ASSERT_TRUE(map_int_int_set(&map, 42, 100));
...
{
    map_int_int map = map_int_int_new(test_heap_alloc);
    TEST_ASSERT_TRUE(map_int_int_set(&map, 1, 10));
...
{
    map_int_int map = map_int_int_new(test_heap_alloc);
    TEST_ASSERT_TRUE(map_int_int_set(&map, 5, 10));
    TEST_ASSERT_TRUE(map_int_int_set(&map, 5, 20));
...
{
    map_int_int map = map_int_int_new(test_heap_alloc);
    TEST_ASSERT_TRUE(map_int_int_set(&map, 7, 77));
...
void test_map_remove_decrements_count(void)
{
    map_int_int map = map_int_int_new(test_heap_alloc);
    TEST_ASSERT_TRUE(map_int_int_set(&map, 1, 10));
    TEST_ASSERT_TRUE(map_int_int_set(&map, 2, 20));
...
{
    map_int_int map = map_int_int_new(test_heap_alloc);
    TEST_ASSERT_TRUE(map_int_int_set(&map, 3, 30));
...
void test_map_growth_all_entries_retrievable(void)
{
    ...
    for (int index = 0; index < 32; index++) {
        TEST_ASSERT_TRUE(map_int_int_set(&map, index, index * 10));
    }
...
void test_map_growth_capacity_is_power_of_two(void)
{
    ...
    for (int index = 0; index < 20; index++) {
        TEST_ASSERT_TRUE(map_int_int_set(&map, index, index));
    }
...
{
    map_probe_test map = map_probe_test_new(test_heap_alloc);
    /* All keys hash to slot 7; they fill consecutive slots via linear probing. */
    TEST_ASSERT_TRUE(map_probe_test_set(&map, 100, 1));
    TEST_ASSERT_TRUE(map_probe_test_set(&map, 200, 2));
    TEST_ASSERT_TRUE(map_probe_test_set(&map, 300, 3));
...
void test_map_tombstone_get_finds_later_entry(void)
{
    ...
    TEST_ASSERT_TRUE(map_probe_test_set(&map, 10, 1));
    TEST_ASSERT_TRUE(map_probe_test_set(&map, 20, 2));
    TEST_ASSERT_TRUE(map_probe_test_set(&map, 30, 3));
...
void test_map_tombstone_reused_on_insert(void)
{
    ...
    TEST_ASSERT_TRUE(map_probe_test_set(&map, 40, 4));
...
{
    map_int_float map = map_int_float_new(test_heap_alloc);
    TEST_ASSERT_TRUE(map_int_float_set(&map, 1, 3.14F));
    const float *result = map_int_float_get(&map, 1);
    TEST_ASSERT_NOT_NULL(result);
    TEST_ASSERT_FLOAT_WITHIN(0.001F, 3.14F, *result);
    map_int_float_free(&map);
}

void test_map_int_bool_set_and_get(void)
{
    map_int_bool map = map_int_bool_new(test_heap_alloc);
    TEST_ASSERT_TRUE(map_int_bool_set(&map, 5, true));
    const bool *result = map_int_bool_get(&map, 5);
    TEST_ASSERT_NOT_NULL(result);
    TEST_ASSERT_TRUE(*result);
    map_int_bool_free(&map);
}

void test_map_int_i64_set_and_get(void)
{
    map_int_i64 map = map_int_i64_new(test_heap_alloc);
    TEST_ASSERT_TRUE(map_int_i64_set(&map, 99, 123456789012345LL));
    const int64_t *result = map_int_i64_get(&map, 99);
    TEST_ASSERT_NOT_NULL(result);
    TEST_ASSERT_EQUAL_INT64(123456789012345LL, *result);
    map_int_i64_free(&map);
}
...
{
    map_int_int map = map_int_int_new(test_heap_alloc);
    TEST_ASSERT_TRUE(map_int_int_set(&map, 7, 70));
...
void test_map_arena_allocator(void)
{
    ...
    map_int_int map = map_int_int_new(alloc);
    for (int index = 0; index < 10; index++) {
        TEST_ASSERT_TRUE(map_int_int_set(&map, index, index * 2));
    }
