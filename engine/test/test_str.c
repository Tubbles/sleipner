...
{
    Str str = {0};
    TEST_ASSERT_TRUE(str_from_cstr(&ctx, &str, "hello"));
...
{
    Str str = {0};
    TEST_ASSERT_TRUE(str_from_cstr(&ctx, &str, ""));
...
void test_str_from_strv_len(void)
{
    ...
    Str str = {0};
    TEST_ASSERT_TRUE(str_from_strv(&ctx, &str, strv));
    TEST_ASSERT_EQUAL_size_t(5, str.len);
    str_free(&ctx, &str);
}
...
void test_str_from_strv_subview(void)
{
    ...
    Str str = {0};
    TEST_ASSERT_TRUE(str_from_strv(&ctx, &str, head));
...
{
    Str str = {0};
    TEST_ASSERT_TRUE(str_from_cstr(&ctx, &str, "ab"));
    TEST_ASSERT_TRUE(str_push_char(&ctx, &str, 'c'));
    TEST_ASSERT_EQUAL_size_t(3, str.len);
    str_free(&ctx, &str);
}
...
{
    Str str = {0};
    TEST_ASSERT_TRUE(str_from_cstr(&ctx, &str, "foo"));
    TEST_ASSERT_TRUE(str_append_cstr(&ctx, &str, "bar"));
...
void test_str_append_strv_content(void)
{
    ...
    TEST_ASSERT_TRUE(str_append_strv(&ctx, &str, suffix));
...
{
    Str str = {0};
    TEST_ASSERT_TRUE(str_from_cstr(&ctx, &str, "key="));
    Strv rest = strv_from_cstr("type:value");
    (void)strv_split(&rest, ':');
    TEST_ASSERT_TRUE(str_append_strv(&ctx, &str, rest));
...
void test_str_append_grows_beyond_initial_cap(void)
{
    ...
    for (int index = 0; index < 32; index++) {
        TEST_ASSERT_TRUE(str_push_char(&ctx, &str, 'x'));
    }
...
{
    Str str = {0};
    str_free(&ctx, &str); /* must not crash */
}
