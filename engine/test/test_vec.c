...
{
    vec_int vector = {0};
    TEST_ASSERT_TRUE(vec_int_push(&vector, 1, NULL));
    TEST_ASSERT_EQUAL_INT(1, vector.count);
    vec_int_free(&vector, NULL);
}

void test_vec_push_single_value_readable(void)
{
    vec_int vector = {0};
    TEST_ASSERT_TRUE(vec_int_push(&vector, 42, NULL));
...
{
    vec_int vector = {0};
    TEST_ASSERT_TRUE(vec_int_push(&vector, 10, NULL));
    TEST_ASSERT_TRUE(vec_int_push(&vector, 20, NULL));
    TEST_ASSERT_TRUE(vec_int_push(&vector, 30, NULL));
...
{
    vec_int vector = {0};
    bool result = vec_int_push(&vector, 7, NULL);
...
void test_vec_push_up_to_capacity_no_extra_alloc(void)
{
    ...
    for (int index = 0; index < VEC_INITIAL_CAPACITY; index++) {
        TEST_ASSERT_TRUE(vec_int_push(&vector, index, NULL));
    }
...
void test_vec_growth_preserves_existing_values(void)
{
    ...
    for (int index = 0; index < push_count; index++) {
        TEST_ASSERT_TRUE(vec_int_push(&vector, index * 10, NULL));
    }
    for (int index = 0; index < push_count; index++) {
...
void test_vec_clear_resets_count_to_zero(void)
{
    vec_int vector = {0};
    TEST_ASSERT_TRUE(vec_int_push(&vector, 1, NULL));
    TEST_ASSERT_TRUE(vec_int_push(&vector, 2, NULL));
...
{
    vec_int vector = {0};
    TEST_ASSERT_TRUE(vec_int_push(&vector, 99, NULL));
    vec_int_clear(&vector);
    TEST_ASSERT_TRUE(vec_int_push(&vector, 77, NULL));
    TEST_ASSERT_EQUAL_INT(1, vector.count);
...
void test_vec_free_allows_repush(void)
{
    ...
    TEST_ASSERT_TRUE(vec_int_push(&vector, 1, NULL));
    vec_int_free(&vector, NULL);
    TEST_ASSERT_TRUE(vec_int_push(&vector, 55, NULL));
    TEST_ASSERT_EQUAL_INT(1, vector.count);
...
{
    vec_int vector = {0};
    TEST_ASSERT_TRUE(vec_int_push(&vector, 100, NULL));
    TEST_ASSERT_TRUE(vec_int_push(&vector, 200, NULL));
    TEST_ASSERT_TRUE(vec_int_push(&vector, 300, NULL));
...
{
    vec_float vector = {0};
    TEST_ASSERT_TRUE(vec_float_push(&vector, 1.5F, NULL));
    TEST_ASSERT_TRUE(vec_float_push(&vector, 2.5F, NULL));
    TEST_ASSERT_EQUAL_FLOAT(1.5F, vector.data[0]);
    TEST_ASSERT_EQUAL_FLOAT(2.5F, vector.data[1]);
    vec_float_free(&vector, NULL);
}

void test_vec_bool_push_and_read(void)
{
    vec_bool vector = {0};
    TEST_ASSERT_TRUE(vec_bool_push(&vector, true, NULL));
    TEST_ASSERT_TRUE(vec_bool_push(&vector, false, NULL));
    TEST_ASSERT_TRUE(vector.data[0]);
    TEST_ASSERT_FALSE(vector.data[1]);
    vec_bool_free(&vector, NULL);
}
...
void test_vec_i64_push_and_read(void)
{
    ...
    TEST_ASSERT_TRUE(vec_i64_push(&vector, large_value, NULL));
    TEST_ASSERT_EQUAL_INT64(large_value, vector.data[0]);
    vec_i64_free(&vector, NULL);
}

void test_vec_u32_push_and_read(void)
{
    vec_u32 vector = {0};
    TEST_ASSERT_TRUE(vec_u32_push(&vector, 0xDEADBEEFU, NULL));
    TEST_ASSERT_EQUAL_UINT32(0xDEADBEEFU, vector.data[0]);
    vec_u32_free(&vector, NULL);
}
...
{
    vec_point vector = {0};
    TEST_ASSERT_TRUE(vec_point_push(&vector, (Point){.x_pos = 3, .y_pos = 7}, NULL));
    TEST_ASSERT_TRUE(vec_point_push(&vector, (Point){.x_pos = 9, .y_pos = 1}, NULL));
...
void test_vec_struct_growth_preserves_fields(void)
{
    ...
    for (int index = 0; index < push_count; index++) {
        TEST_ASSERT_TRUE(vec_point_push(&vector, (Point){.x_pos = index, .y_pos = index * 2}, NULL));
    }
    for (int index = 0; index < push_count; index++) {
