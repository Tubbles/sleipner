...
void test_arena_alloc_alignment(void)
{
    ...
    void *aligned = arena_alloc(&arena, 16, 8);
...
void test_arena_alloc_returns_null_when_full(void)
{
    ...
    void *first = arena_alloc(&arena, 48, 1);
    TEST_ASSERT_NOT_NULL(first);

    /* Not enough room for 32 more bytes */
    void *second = arena_alloc(&arena, 32, 1);
    TEST_ASSERT_NULL(second);

    /* But smaller allocation still fits */
    void *third = arena_alloc(&arena, 16, 1);
...
void test_arena_reset(void)
{
    ...
    void *pointer = arena_alloc(&arena, 128, 1);
...
void test_arena_snapshot_restore(void)
{
    ...
    /* Write some data */
    int *values = arena_alloc(&arena, 4 * sizeof(int), _Alignof(int));
    values[0] = 10;
    values[1] = 20;
    values[2] = 30;
    values[3] = 40;
