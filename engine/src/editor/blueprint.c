                                          ...
                                          const InputState *input)
{
     ...
     * masked by the plain Left/Right navigate bindings. S8.7h2a: the per-author op
     * log handles the press in a networked session (editor_session_*, core.c); when
     * it does not (offline, or a host with an empty op log) the local snapshot step
     * runs. Blueprint mode is structural, so a NET_CLIENT never reaches here (client
     * structural modes are blocked) and a NET_HOSTING peer's blueprint edit is not
     * op-logged -- the empty-log host fallback runs the local step and the f2
     * trigger resyncs. */
