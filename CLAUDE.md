...
- **TODO.md workflow.** After completing a task, first check GitHub Actions CI — there are two workflows (CI and Android), so use `gh run list --limit 2` to see both. Use `gh run view` on any failures and present them as items to address. Then check `TODO.md` for pending items. If there are items, ask the user if they want to work through them. One item at a time, one commit per item, remove items as they're done, keep the file.
