...
Sleipner is a top-down Zelda-like action RPG written in C using raylib. The game is controller-driven and targets Linux x86_64 and Windows x86_64. It also runs on Android phones via the "Game Native" app, which uses Proton to run a Windows PE binary — so the Windows build target is a cross-compiled `.exe` (via MinGW), not an Android NDK build.
...
- **Mobile target:** The game runs on Android via Game Native (Proton). This means we cross-compile a Windows `.exe` via MinGW — no Android-specific code. Keep performance reasonable for emulated execution (avoid heavy compute, prefer simple draw calls).
...
- **TODO.md workflow.** After completing a task, first check GitHub Actions CI — there are two workflows (CI and Android). Check each with `gh run list --workflow ci.yml --limit 1` and `gh run list --workflow android.yml --limit 1`. Use `gh run view` on any failures and present them as items to address. Then check `TODO.md` for pending items. If there are items, ask the user if they want to work through them. One item at a time, one commit per item, remove items as they're done, keep the file.
