...
- **TODO.md workflow.** After completing a task, first check the latest GitHub Actions CI run with `gh run list --limit 1` and `gh run view` — present any CI failures as items to address. Then check `TODO.md` for pending items. If there are items, ask the user if they want to work through them. One item at a time, one commit per item, remove items as they're done, keep the file.
