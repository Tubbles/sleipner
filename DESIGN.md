...
### Inventory & Items
- Resolved by D25 (S6.8a): unlimited list (no slot cap), stackable by count, `give_item`/`has_item`/`remove_item` work off a name -> count map (`ItemSet`, `ProgressionState.items`) rather than item entities. Pickup toast is S6.8b.
