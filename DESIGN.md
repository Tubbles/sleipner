...
- [ ] Combat: `ACTION_ATTACK` activation, knockback, ranged/projectiles (S6.10b-d)
- [ ] Inventory & items
- [x] Dialogue system
- [ ] Level transitions
...
- Knockback direction/distance — resolved by D26 (impulse along attacker->target, `knockback` attr = distance in px, decaying over 0.15s via a per-entity offset field) but not yet implemented (S6.10c).
- Ranged attacks — resolved by D26 (projectile blueprints via `spawn:` + a straight-line mover behavior) but not yet implemented (S6.10d).
