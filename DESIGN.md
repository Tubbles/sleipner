...
- [x] Host-authoritative session skeleton + JOIN registration + INPUT flow, completing S8.4a of the open-work master plan (`work/open-work-master-plan.md`): `network.h` gains `NET_CLIENT` on `NetMode` -- a joined client, distinct from `NET_JOINING` ("picked a host, about to connect"). Lifecycle: `NET_JOINING` -> `NET_CLIENT` the moment `net_session.c`'s `network_client_tick` first observes `NET_JOINING` and sends `MSG_JOIN`; S8.4a's acceptance is IMPLICIT (the client assumes it's in as soon as it sends JOIN, the host separately hash-verifies and registers or refuses without replying either way) -- S8.4c is where a real accept/reject handshake arrives. `NetClient {NetAddr addr; int player_id; InputState last_input; bool active;}` in a fixed `clients[NET_MAX_CLIENTS]` (4) + `client_count` on `NetworkState`, host-side only. `network_client_send_join(NetworkState*, uint64_t local_gamedata_hash)` encodes/sends `MSG_JOIN` (gamedata hash + `host_name` doubling as the client's own display name -- one "this peer's name" field rather than a second one, since only one role is ever meaningful per `NetworkState`) to `join_target`; `network_client_send_input(NetworkState*, const InputState*)` sends this tick's input the same way. `network_host_receive(NetworkState*, uint64_t local_gamedata_hash)` drains every pending packet: a `MSG_JOIN` is hash-verified (`protocol_join_verify`) -- a match registers the sender's `NetAddr` with the next `player_id` (1..N in join order, id 0 reserved for the host's own `local:0` player; a no-op, not a duplicate, if that addr already registered) via file-local `register_client`, a mismatch is silently refused; a `MSG_INPUT` from an already-registered addr overwrites that client's `last_input` via file-local `handle_input_datagram`, from an unregistered addr is ignored. `network_find_client_by_player_id` is the read side. `net_session.{h,c}` (new files) hold the two GameState-level tick hooks -- split out from `network.c` because `network.h` is deliberately decoupled from `game.h` (the reverse include would cycle): `network_host_tick(GameState*)` is a no-op unless `NET_HOSTING`, otherwise calls `network_host_receive` against the new `GameState.gamedata_hash` field (`uint64_t`, computed by `game_load_gamedata` via `gamedata_content_hash(params.toml_string)` on every call regardless of parse success); `network_client_tick(GameState*, const InputState*)` is a no-op unless `NET_JOINING`/`NET_CLIENT`, otherwise sends JOIN-then-flip-to-`NET_CLIENT` on first observing `NET_JOINING`, then sends INPUT every tick while `NET_CLIENT`. `frame.c`'s `run_active_frame` calls both, unconditionally of `editor_mode`, right before `game_update` -- same "doesn't care whether the player happens to be in the editor" reasoning `tick_network`'s own doc comment gives -- and passes the raw per-frame `input` (not the fade-suppressed `game_input`), since a client's own local transition shouldn't also suppress what it forwards to the host. `game.c`'s `input_for_entity` (the S6.9a/D39 seam) gains a `const NetworkState*` parameter and a `"network:<id>"` `input_source` branch: `strv_starts_with_cstr` matches the prefix, `strtol` parses the id, `network_find_client_by_player_id` resolves it to that client's `last_input` (idle input if no such client is registered -- unset id, wrong id, or a non-hosting `GameState`); `"local:0"` behavior is unchanged. `behavior_player` now passes `&state->network` through. Ripple: S8.4a's implicit JOIN-on-first-tick means `NET_JOINING` is no longer the long-lived "still browsing, discovery keeps ticking" state S8.3b described -- it now promotes to `NET_CLIENT` within the very first `run_active_frame`, so `game.c`'s `tick_network` (whose `NET_DISCOVERING`/`NET_JOINING` branch drives `discovery_client_tick`) never observes `NET_JOINING` past that first frame either; the old regression test asserting a stale join-list host gets evicted after `DISCOVERY_TIMEOUT_SECONDS` of `NET_JOINING` was replaced with `test_integration_network_joining_immediately_connects_and_stops_discovering`, asserting the new reality (`NET_CLIENT` after one frame, stale host never evicted because discovery stopped ticking). Tested in `engine/test/integration_test.c`'s `test_integration_client_input_moves_player_on_host`: two full `TestGame`s (HOST, CLIENT) wired over one `net_loopback.h` `LoopbackNetwork`, driven only through `test_advance_frame` (the real `frame_update` -> `run_active_frame` path, never calling `network_client_tick`/`network_host_tick` directly from the test body) -- host gamedata seeds one `local:0` player and one `network:1` player at distinct positions; twenty frames of the client holding right moves the host's `network:1` entity right by the expected `speed * delta_time * frames` while the host's own `local:0` entity (fed no local input) stays put, and a rogue endpoint's `MSG_JOIN` with a deliberately wrong gamedata hash is confirmed refused (`client_count` stays at 1). Verified by temporarily stubbing the `"network:<id>"` branch to return idle input: the movement assertion failed (`Expected 226.6667 Was 200`) while everything else stayed green; reverted before committing. Full suite (43 targets, 246 Unity cases) passes, no new ASan/UBSan/LSan report files, and the exact CI `clang-tidy` glob is clean. State sync back to the client (attribute deltas per tick, full snapshot on join) is S8.4b; the reliable event sub-channel is S8.4c. (S8.4a)
- [x] Host-to-client state sync via SNAPSHOT (full state on join) + per-tick DELTA, completing S8.4b of the open-work master plan (`work/open-work-master-plan.md`): what's synced is every entity in the host's current level -- `entity->position` (a `Vector2` FIELD) plus every entry in `entity->attrs` (instance attrs: health, state, direction, ...; NOT `entity->persisted_attrs`, the pre-play authored baseline, and NOT runtime-only render fields like `anim_row`/`frame_index`, which the client re-derives locally via the existing generic animation pass). Position rides `net_protocol.h`'s existing `AttrRecord` stream (S8.2) under two reserved, documented attr names (`net_session.h`'s `NETWORK_ATTR_POS_X`/`_Y`, both `ATTR_FLOAT`) rather than a second wire shape -- zero protocol change, `network_client_apply_state` (below) recognizes them and writes `entity->position` instead of calling `attr_set_float`. `net_session.c` gains the builder: file-local `push_entity_sync_records`/`build_level_sync_records` walk a `Level`'s entities into a scratch-arena `vec_attr_record` (new `VEC_DECL(attr_record, AttrRecord)` in `net_protocol.h`, the type's first vec since S8.2 only ever needed fixed caller-owned arrays); file-local `send_sync_records` chunks that vec into `NETWORK_SYNC_RECORDS_PER_PACKET` (20, a conservative budget documented against `NET_MAX_PACKET_SIZE` and this codebase's actual attr-name lengths) -sized packets via a `protocol_encode_snapshot_packet`/`_delta_packet` function pointer, so a level with more entities/attrs than one packet holds sends across several packets instead of overflowing one -- a chunk that still fails to encode (`protocol_encode_*_packet`'s own bounds-check) is silently dropped, never a buffer overflow, same fire-and-forget contract as every other `network_*` send. `network_host_send_snapshot(GameState*, const NetClient*)` builds once and sends every chunk as `MSG_SNAPSHOT` to one client; `network_host_broadcast_delta(GameState*)` builds once and sends every chunk as `MSG_DELTA` to every active client. v1 simplification, documented at the call sites: DELTA carries the exact same full synced-state content as SNAPSHOT every tick, not a diff against last tick -- real per-attr diffing is a bandwidth optimization deferred to TODO.md, fine on the LAN this pre-alpha targets, and it means a client's view can never permanently drift, only lag by at most one tick. `net_session.c`'s `network_host_tick` now diffs `client_count` before/after `network_host_receive` and calls `network_host_send_snapshot` for every newly-registered client's tail slice (no signature change to `network.c`'s `register_client`, keeping `network.h` decoupled from `game.h`); `network_client_tick`'s `NET_CLIENT` branch now also calls file-local `network_client_receive_state`, which drains every pending packet and applies whichever decodes as `MSG_SNAPSHOT` or `MSG_DELTA` via `network_client_apply_state(GameState*, const AttrRecord*, size_t)` -- entity ids resolved via `level_find_entity_by_id` (an id the client's level doesn't have, e.g. before both sides share a level, is silently skipped), every record's Strv name (and, for `ATTR_STRING`, value) copied through a stack buffer into a null-terminated `const char *` (`strv_copy_to_cstr`) and handed to `attr_set_float`/`_int`/`_bool`/`_string`, which itself deep-copies into `state->gamedata_arena` -- nothing borrowed from the receive buffer is ever retained. Client frame shape (`frame.c`'s `run_active_frame`): a `NET_CLIENT` frame no longer calls `game_update` at all -- running the host's own behavior dispatch/combat/collision/rule evaluation locally would duplicate its authority over data it already owns and could fire side effects (sounds, toasts, dialogue) the host never intended -- instead it calls new `game_update_client_render(GameState*, float)` (`game.c`), which only advances the generic animation pass (`advance_entity_animation`, driven by the attrs `network_client_apply_state` just applied) and the camera follow target, mirroring `game_update`'s own calls to both while skipping everything else; `NET_OFFLINE`/`NET_HOSTING` frames are unchanged, and a `NET_HOSTING` frame now calls `network_host_broadcast_delta` right after `game_update` so the broadcast reflects this tick's freshest simulated state. Menu/UI handling needed no change -- `frame_update` already branches away from `run_active_frame` entirely whenever a menu/modal screen is open, for every mode. One pre-existing-struct fix surfaced by clang-tidy's `clang-analyzer-optin.performance.Padding` once `net_session.c` started constructing many more `AttrRecord`s than `net_protocol.c`/`net_protocol_test.c` ever had: `AttrRecord`'s field order was `entity_id, name, type, value` (8 padding bytes); reordered to `name, value, entity_id, type` (0 padding) -- every construction site already used designated initializers, so this is a layout-only change, confirmed by re-running the full suite. Tested in `engine/test/integration_test.c`: `test_integration_delta_converges_client_view` drives the HOST's own `local:0` "hero" with held-right input for 20 ticks (plus one client-only catch-up tick to drain the last in-flight `DELTA`) and asserts the CLIENT's copy of that same entity converges to the HOST's position exactly (bit-exact float round-trip) and that the synced `state="walk"` attr (written by `update_entity_anim_attrs`, game.c) also reaches the client; `test_integration_join_snapshot_equivalence` diverges the HOST's state (both entities moved, both given a `health` instance attr) BEFORE a fresh client ever joins, then asserts full-state equivalence for EVERY entity in the level (not just the player) after join. Both reuse S8.4a's `host_session_gamedata`/two-`TestGame`-over-`net_loopback.h` shape, driven only through `test_advance_frame`. Verified by temporarily making `network_client_apply_state` return immediately: both new tests failed (`Expected 126.6667 Was 100`, `Expected 150 Was 100`) while `test_integration_client_input_moves_player_on_host` still passed; reverted before committing. Full suite (43 targets, 250 Unity cases) passes, no new ASan/UBSan/LSan report files, and the exact CI `clang-tidy` glob is clean. The reliable event sub-channel (rule triggers, item pickups delivered exactly once even across a dropped packet) is S8.4c. (S8.4b)
- [x] Reliable event sub-channel via seq + ack-bitfield resend, completing S8.4c and all of S8.4 (host-authoritative loop) of the open-work master plan (`work/open-work-master-plan.md`): new `net_reliable.{h,c}` implements one `ReliableChannel` per CONNECTION DIRECTION -- a fixed send window of `RELIABLE_WINDOW` (64) `ReliableSendSlot {sequence_number, packet[RELIABLE_EVENT_PAYLOAD_MAX], packet_length, seconds_since_send, awaiting_ack}` entries (a pre-encoded `MSG_EVENT` packet ready to hand straight to `net_send`, so a resend never re-derives anything) plus receive-side `has_received_any`/`highest_received_sequence`/`received_bitfield`. `RELIABLE_EVENT_PAYLOAD_MAX` (128) is deliberately NOT `net.h`'s `NET_MAX_PACKET_SIZE` (1400) -- a reliable event is a small discrete notification, not a level-sized SNAPSHOT/DELTA payload, and 128 (`PACKET_HEADER_SIZE` + `EventRecord`'s two `int32`s + a u16 length prefix + up to `rule.h`'s own `MAX_ARG` (64) argument bytes, rounded up) keeps a whole `GameState`'s reliable-channel footprint in the tens of KB rather than the ~450KB a `NET_MAX_PACKET_SIZE`-sized slot per channel (`NET_MAX_CLIENTS` host-side + one client-side, `GameState` is stack-allocated in both `main.c` and every test's `TestGame`) would cost. `reliable_send(channel, transport, dest, event)` assigns `next_send_sequence` into slot `sequence_number % RELIABLE_WINDOW` and transmits once; sending the window's 65th unacked event EVICTS the oldest still-unacked slot (silently overwritten, never delivered) rather than blocking the caller or growing unbounded -- documented backpressure, justified the same way `JoinList`/`NetClient`'s own silent-drop-past-cap contracts already are, since 64 simultaneously in-flight discrete notifications is far beyond any expected pre-alpha session. `reliable_tick(channel, transport, dest, delta_time)` ages every occupied slot and resends (the exact same bytes) any slot past `RELIABLE_RESEND_SECONDS` (0.1F). `reliable_on_ack(channel, AckMessage)` clears every slot the ack's `ack_seq`/`ack_bitfield` covers (takes the whole `AckMessage` rather than its two `uint32` fields separately so no two adjacent parameters share a type, the same `bugprone-easily-swappable-parameters` dodge `net_protocol.c`'s `write_be_uint`/`packet_begin` already use). `reliable_on_receive(channel, sequence_number) -> bool is_new` updates the highest-seq/bitfield window using the exact bit convention `PROTOCOL_ACK_BITFIELD_BITS` already documents for `AckMessage.ack_bitfield` (bit N acks `highest - 1 - N`) -- every shift is guarded against `>=32`-width UB via a ternary whose untaken branch is never evaluated, not a runtime bounds check -- and returns whether the caller should actually deliver the event; a sequence more than 32 behind the tracked window is conservatively treated as a duplicate. `reliable_make_ack(channel)` packages the receive-side state as an `AckMessage` ready to send as `MSG_ACK`. `network.{h,c}` wires the transport-level fan-out: `NetClient` gains an `event_channel` (HOST->this-client), `NetworkState` gains a `host_event_channel` (CLIENT's own receive side) plus `last_delivered_event_type`/`_entity_id`/`delivered_event_count` bookkeeping; `network_broadcast_reliable_event`/`network_host_tick_reliable_channels`/`network_client_send_ack` are the new primitives, and `network_host_receive` gained a `MSG_ACK` case (file-local `handle_ack_datagram`, applies an incoming ack to the sending client's own `event_channel`, ignored from an unregistered address same as `MSG_INPUT`) alongside its existing `MSG_JOIN`/`MSG_INPUT` handling; `network_stop` now also resets `host_event_channel` and the delivered-event bookkeeping. `net_session.{h,c}` wires the game-level policy: `network_host_tick` gained a `delta_time` parameter (threaded from `frame.c`'s `run_active_frame`, which already had it) so it can call `network_host_tick_reliable_channels` after `network_host_receive`; the one concrete event this slice wires end-to-end is `NETWORK_EVENT_PLAYER_JOINED` (`net_session.h`) -- `network_host_tick` reliable-sends it to every active client (including the one that just joined) whenever a new client registers. This is a session-management notification, NOT a real `rule.h` `TriggerType`/`TriggerEvent` (the brief's own suggested `TRIGGER_DEFEAT`/transition-notification candidates): `game_update`'s `trigger_events` vec (`game.c`) is scratch-arena-scoped and fully drained by `rules_evaluate_batch` inside that same call, with no hook point any caller outside `game.c` can currently observe, so routing a real gameplay trigger over this channel needs that hook added first -- broader than the reliable-channel brief itself, tracked in TODO.md as a named follow-up. `network_client_tick`'s `NET_CLIENT` branch now also applies any newly-delivered `MSG_EVENT` (file-local `network_client_receive_event`/`network_client_apply_event`, dedup'd via `reliable_on_receive` before "applying" -- currently just the bookkeeping fields above, since `NETWORK_EVENT_PLAYER_JOINED` has no gameplay effect of its own yet) and sends the current ack (`network_client_send_ack`, a no-op before anything has ever arrived). One pre-existing bug surfaced by this slice, fixed in `net_protocol.c`: `packet_writer_write_bytes` called `memcpy` unconditionally even when `count` is 0, which is undefined behavior if the caller's `bytes` pointer is null (exactly what `EventRecord.argument = (Strv){0}` produces for `NETWORK_EVENT_PLAYER_JOINED`'s empty argument) -- `memcpy`'s `nonnull` parameter attribute makes this UB regardless of the zero count; fixed by skipping the `memcpy` call entirely when `count == 0`, confirmed by a UBSan report (`net_protocol.c:100:43: null pointer passed as argument 2`) before the fix and a clean run after. Tested in new `engine/test/net_reliable_test.c` (`net_reliable_test`, registered via `add_unit_test`, whitebox-including `net_reliable.c`/`net_protocol.c`/`net_loopback.c`): resend-until-acked (send seq 0 over a real `net_loopback.h` pair, drain-and-discard the original to simulate loss, `reliable_tick` past the resend timer, drain the resent copy, ack it, prove a further tick sends nothing more); dedup (`reliable_on_receive` on the same sequence twice, and specifically on sequence 0 -- the zero-init default -- to prove `has_received_any` disambiguates "never received" from "received seq 0"); out-of-order bitfield (receive N, N+2, assert the bitfield reflects the N+1 gap, then N+1 arrives and is new, a second N+1 is a duplicate); partial ack (three sends, an ack covering only seq 0 and seq 2 clears exactly those two slots, seq 1 stays `awaiting_ack`); window bound (65 sends with `RELIABLE_WINDOW` = 64 -- the 65th evicts slot 0 without a crash, `send_window[0].sequence_number` ends at 64). `network_test.c`'s `test_stop_resets_every_field_to_offline` extended to pre-seed `host_event_channel`/the delivered-event fields and assert `network_stop` clears them too. `integration_test.c` gained `test_integration_reliable_event_delivers_player_joined_exactly_once`: the same two-`TestGame`-over-`net_loopback.h` shape as S8.4a/b's tests, driven only through `test_advance_frame` (no direct calls to `net_session.c`/`network.c` internals), asserting `client.state.network.delivered_event_count == 1` and `last_delivered_event_entity_id` matches the host-assigned `player_id` after ordinary per-frame ticking (no packet loss simulated -- this test proves the wiring reaches the game end to end, not the resend timer, which `net_reliable_test.c` already covers in isolation). Verified by temporarily stubbing `reliable_tick` to a no-op: the resend test failed (a `TEST_ASSERT_TRUE(received > 0)` failure, surfaced as a nonzero exit via the resulting ASan leak since the early `TEST_ABORT` skips `loopback_network_free`); and by temporarily stubbing `reliable_on_receive` to always return `true`: four tests failed (dedup, first-call-is-new, out-of-order, partial-ack) while the resend and window-bound tests still passed; both reverted before committing. Full suite (44 targets) passes, no new ASan/UBSan/LSan report files, and the exact CI `clang-tidy` glob is clean. (S8.4c)
- [x] Client-side render-only interpolation between the last two synced positions, no prediction, completing S8.5 of the open-work master plan (`work/open-work-master-plan.md`): S8.4b applies every SNAPSHOT/DELTA straight onto `entity->position`, which would make a NET_CLIENT's remote entities visibly snap once per received update; S8.5 keeps `entity->position` authoritative but adds a render-only lerp window so the pixels on screen move smoothly between updates instead. `entity.h` gains three runtime-only fields on `Entity` (same footing as `patrol_phase`/`iframe_timer` -- NOT emitted to TOML): `Vector2 interp_from`/`interp_to` (the last two synced positions) and `float interp_elapsed` (seconds since the window last shifted). `entity_init` seeds `interp_from`/`interp_to` to the spawn position and `interp_elapsed` to a new sentinel, `ENTITY_INTERP_NEVER_SYNCED` (-1.0F, below any real elapsed time), so a client entity renders sanely at spawn before its first sync arrives. `NETWORK_INTERP_INTERVAL_SECONDS` (0.1F, `entity.h`) is a deliberately conservative constant, not a measured cadence -- S8.4b's `network_host_broadcast_delta` sends a DELTA every single hosting frame with no dedicated network tick rate to size this against, so 0.1F comfortably outlasts one frame at any plausible host frame rate, smoothing host frame-time jitter and UDP reordering without ever extrapolating. `entity_render_position(const Entity*)` (`entity.c`) is the pure lerp: `lerp(interp_from, interp_to, clamp01(interp_elapsed / NETWORK_INTERP_INTERVAL_SECONDS))`, unit-tested directly (`entity_test.c`, bare `Entity` structs, no window/blueprint needed) at elapsed 0 (reports `interp_from` unchanged), half the interval (exact midpoint), and past the interval (clamps at `interp_to`, sampled at 2x the interval to prove no overshoot -- the "no prediction" half of the brief). `net_session.c`'s `apply_sync_record` shifts the window from its `NETWORK_ATTR_POS_Y` branch (POS_X always precedes POS_Y for the same entity, `push_entity_sync_records`, so POS_Y is where the complete new `(x, y)` pair is first known): the new file-local `shift_interp_window` sets `interp_from` to whatever `entity_render_position` currently reports ON SCREEN (so a fresh sync never causes a visible jump) and `interp_to`/`interp_elapsed` to the new position/0 -- except the very first call for an entity (`interp_elapsed < 0`, the sentinel), which collapses `interp_from`/`interp_to` onto the new position and sets `interp_elapsed` to the interval itself, so `entity_render_position` reports exactly that position immediately, no lerp-from-spawn artifact. `entity_draw_position` (`entity.h`/`.c`) gained an explicit `Vector2 base_position` parameter in place of always reading `entity->position` internally, so its three call sites (`main.c`'s `draw_animated_entity`, `draw_entity`, and the player debug-outline draw in `draw_debug_collision_boxes`) can each pick the base with a one-line switch: `state->network.mode == NET_CLIENT ? entity_render_position(entity) : entity->position` -- host/offline rendering is byte-for-byte unchanged, since `NetworkState.mode` zero-inits to `NET_OFFLINE`. `game_update_client_render` (game.c, NET_CLIENT-only, see its own doc comment) now also advances every entity's `interp_elapsed` by `delta_time` each frame, and its camera-follow call switched from the player's raw `position` to `entity_render_position(player)` -- safe unconditionally (no extra mode check needed) since this function itself only ever runs under `NET_CLIENT` -- so the camera doesn't jitter in lockstep with SNAPSHOT/DELTA arrival. Tested in `engine/test/integration_test.c`'s `test_integration_client_interpolates_between_snapshots`: reuses S8.4b's `host_session_gamedata`/two-`TestGame`-over-`net_loopback.h` shape, but deliberately PARKS the client (stops calling `test_advance_frame` on it) while the host alone ticks 15 held-right frames -- `net_loopback.h`'s 32-slot inbox queues every broadcast DELTA unread, and since `interp_elapsed` only advances inside a client's own tick, it stays frozen too. Waking the client for exactly one tick drains the whole 15-packet burst in a single `network_client_receive_state` pass; because `interp_elapsed` never advances mid-drain, every intermediate `shift_interp_window` call anchors `interp_from` to the SAME pre-park position, collapsing the burst into exactly the two-point "snapshot A / snapshot B" window the brief describes -- letting the test assert a precise, strictly-between render position (`TEST_ASSERT_GREATER_THAN_FLOAT`/`_LESS_THAN_FLOAT` against the pre-park and post-burst positions) rather than a fuzzy one, then drains ten more solo client ticks to prove the render position converges on, and never overshoots past, the authoritative one once the interval elapses. Verified by temporarily stubbing `entity_render_position` to return `entity->position` unconditionally: all four new unit tests and the new integration test failed (`Expected 100 Was 0`, `Expected 120 to be less than 120`) while the rest of the suite stayed green; reverted before committing. Full suite (44 targets) passes, no new ASan/UBSan/LSan report files, and the exact CI `clang-tidy` glob is clean. (S8.5)
- [x] Multiple player entities (host spawns one per join, `input_source = network:<id>`) and per-player cameras (each peer's own camera/HUD follows its own player), completing S8.6 of the open-work master plan (`work/open-work-master-plan.md`) and, with it, Stage 8 through S8.6 -- S8.7 (collaborative editor) gets its own plan document next, per the master plan's own note. Two deliverables: a real JOIN-accept handshake (S8.4a shipped only the implicit-acceptance half), and the dynamic per-join spawn itself, which turned out to need a third piece the brief hadn't spelled out -- a way for a CLIENT to materialize an entity the HOST created after that client's own initial parse. All three below. **JOIN-accept**: `MSG_JOIN_ACCEPT` (`net_protocol.h`/`.c`, `JoinAcceptMessage {int32 player_id}`, round-trip tested in `net_protocol_test.c`) is sent by `network.c`'s `handle_join_datagram` for every hash-verified JOIN (new registration or re-JOIN refresh alike, so a client whose first accept was lost still converges on its next resend) via a `dest`/`player_id` fire-and-forget send, never for a refused one. `NetworkState.local_player_id` (`network.h`) is the CLIENT-side field that reply fills in. `network_client_tick` (`net_session.c`) no longer flips `NET_JOINING` -> `NET_CLIENT` the instant `MSG_JOIN` is sent (`NetMode`'s own doc comment, `network.h`, documents the full new handshake) -- it resends JOIN every tick while still JOINING and only advances once `network_client_receive_state` (now called unconditionally of mode, so an accept and a SNAPSHOT that both already arrived apply in the SAME drain pass -- no packet discarded by a narrower "just look for the accept" loop) decodes a `MSG_JOIN_ACCEPT` and sets `local_player_id`/`mode` itself. **Per-join spawn**: `net_session.c`'s `spawn_network_player` (called from `network_host_tick`'s existing newly-joined-clients loop, BEFORE the SNAPSHOT send so it's included) clones the HOST's own local player's blueprint (`game_get_player` + `entity_resolve_blueprint` -- always resolves to the local:0 entity even after N previous spawns, since `player_index` is fixed at the first behavior==player entity and every dynamic spawn is appended after it) via `level_spawn_entity` at that local player's CURRENT position (documented choice, not the only one considered: an authored player-start attr or a level spawn-point system are the alternatives), stamps `input_source = "network:<id>"` as an instance attr (picked up unchanged by the existing `input_for_entity` routing, D39/S8.4a), and rebuilds count-parallel tracking via a newly-extracted `game_respawn_rebuild_tracking` (`game.c`, moved here from `frame.c`'s file-local `respawn_rebuild_tracking` so S6.6's editor spawn action and this host-side spawn share one overlap-edge-preserving rebuild instead of two copies). **Client-side materialization**: S8.4b/S8.5's sync (`network_client_apply_state`) had always silently skipped any `AttrRecord.entity_id` the receiving client's `level_find_entity_by_id` didn't already know -- true for every entity synced before S8.6, since both sides parsed identical gamedata at load time and ids trivially lined up, but the FIRST thing to ever break that assumption. Fix: a third reserved attr-stream name, `NETWORK_ATTR_BLUEPRINT_NAME` (`net_session.h`, alongside `NETWORK_ATTR_POS_X`/`_Y`), pushed first for every entity on every sync (`push_entity_sync_records`) -- `network_client_apply_state` routes it to the new `ensure_synced_entity_exists`, which no-ops if the id already resolves, otherwise `blueprint_find`s the named blueprint and `level_spawn_entity`s a placeholder (position `(0, 0)`, immediately overwritten by the `POS_X`/`_Y` records that always follow in the same batch; `entity_init`'s `ENTITY_INTERP_NEVER_SYNCED` seed means the first `shift_interp_window` call collapses the S8.5 render-interp window onto that real position too, so the placeholder is never actually rendered) then FORCES its id to match `record->entity_id` (bumping `next_entity_id` past it too) -- necessary, not a style choice: `level_spawn_entity`'s own auto-assignment has no cross-peer coordination, so only an explicit force keeps the entity resolvable by every later record naming the same id, the same way an entity both sides parsed identically already was. Riding the existing full-state-every-sync attr stream (rather than the S8.4c reliable event channel, which was the other candidate) means a client that joins AFTER other players already exist still learns about all of them from its own first SNAPSHOT, with no separate backfill logic needed. **Local-player resolution**: `game_get_local_player`/`_const` (`game.c`/`.h`) resolve THIS peer's own player -- host/offline: exactly `game_get_player` (no scan; `player_index` already means "the local:0 player" and dynamic spawns can never precede it in entity order); `NET_CLIENT`: scans for the entity whose scoped `input_source` matches `NETWORK_INPUT_SOURCE_PREFIX` (moved from a `game.c`-local `#define` to `network.h` so both `game.c` and `net_session.c` share the one literal) plus `local_player_id`, reusing `input_for_entity`'s own prefix+`strtol` parse rather than a fresh `snprintf`-and-compare. Repoints both `camera_update_target` call sites (`game_update`'s host/offline branch, `game_update_client_render`'s client branch) and `main.c`'s `draw_player_hud` (HUD hearts) plus, by the same "this peer's own info" reasoning, `draw_debug_info` (the F3 panel) from `game_get_player` to `game_get_local_player` -- `behavior_chase`'s AI aggro target and `collect_trigger_events`'s interact/enter-overlap detection are deliberately left on `game_get_player` (a host-authoritative "the" player, not a peer-local one; making every connected player able to trigger interacts/overlaps is tracked in TODO.md as follow-up work, since `prev_player_overlaps` would need to become per-player first). **Tests** (`integration_test.c`): `host_session_gamedata` (shared by every S8.4/S8.5 test) drops its pre-authored `remote_hero`/`network:1` entity -- S8.6 makes that connection dynamic, so authoring it statically would fight the new spawn. `test_integration_client_input_moves_player_on_host` and `test_integration_join_snapshot_equivalence` (S8.4a/b) are adapted to locate the dynamically spawned entity via `test_find_entity_by_blueprint_excluding_id` instead of by the old fixture blueprint name, each gaining a short no-input warmup so the now-two-tick JOIN-accept round trip completes before the timed part of each test begins; `test_integration_delta_converges_client_view`, `test_integration_client_interpolates_between_snapshots`, and `test_integration_reliable_event_delivers_player_joined_exactly_once` needed no changes (each already tracked hero specifically, or asserted only on `NetworkState` bookkeeping untouched by S8.6). `test_integration_network_joining_immediately_connects_and_stops_discovering` (S8.4a) is replaced by `test_integration_network_joining_without_accept_keeps_discovering`, sharing `assert_network_mode_ticks_client_and_evicts_stale_host` with the existing `NET_DISCOVERING` case: a `NET_JOINING` client with no responding host now stays `NET_JOINING` forever (never an implicit flip) and so keeps ticking `discovery_client_tick` exactly like `NET_DISCOVERING` does, evicting a stale host after `DISCOVERY_TIMEOUT_SECONDS` -- the pre-S8.4a behavior, true again for exactly this "never accepted" case. New: `test_integration_host_spawns_player_per_join` (host + two sequential clients: the first join produces a second player entity, `input_source = "network:1"`, driven only by that client's own input; the second produces a third, `"network:2"`, and each client's held-right input moves only its own entity, confirmed by asserting the OTHER two entities' positions stay exactly where they were); `test_integration_client_learns_its_player_id` (`NetworkState.local_player_id` matches the host's own `clients[0].player_id`, and `game_get_local_player_const` on the CLIENT resolves an entity distinct from that client's own locally-parsed hero); `test_integration_per_peer_camera_follows_own_player` (host + two clients, each's player diverged in a different direction on a dedicated large, centered-spawn fixture -- `camera_test_gamedata` -- sized so `camera_clamp_target`'s edge-of-level clamping never compresses the separation between the three ground-truth positions, which a first attempt reusing the cramped `host_session_gamedata` level tripped over: proximity comparisons, not exact values, since `camera_update_target`'s own follow-speed lerp makes exact numbers timing-dependent); `test_integration_offline_local_player_matches_player` (single-player: `game_get_local_player` is pointer-identical to `game_get_player`, and the camera-follow behavior itself is unchanged, moving right long enough to clear the level's own camera deadzone then settling so the lerp fully converges). Verified by two temporary stubs, each reverted before committing: (1) no-opping `spawn_network_player`'s call site failed `test_integration_client_input_moves_player_on_host`, `test_integration_join_snapshot_equivalence`, `test_integration_host_spawns_player_per_join` (`Expected 2 Was 1` on entity count), `test_integration_client_learns_its_player_id`, and `test_integration_per_peer_camera_follows_own_player`, while `test_integration_offline_local_player_matches_player` stayed green (no networking involved); (2) forcing `find_local_player_entity` to always return `game_get_player`'s answer failed `test_integration_client_learns_its_player_id` (`Expected 'network:1' Was NULL` -- resolved to the client's own hero, which has no `input_source` instance attr at all) and `test_integration_per_peer_camera_follows_own_player` (`Expected TRUE Was FALSE` -- a client's camera converged toward the host's hero instead of its own player), while every spawn/routing-only test and the offline test stayed green. Full suite (44 targets) passes, no new ASan/UBSan/LSan report files, and the exact CI `clang-tidy` glob is clean. This completes S8.1-S8.6; S8.7 (collaborative editor) gets its own plan document next. (S8.6)
- [x] Multiple player entities with independent input sources (host spawns one per join -- see S8.6 bullet below)
- [x] Per-player camera (each client follows own player -- see S8.6 bullet below)
- [x] Host-authoritative game loop (host simulates, clients send inputs)
- [x] State sync: attribute deltas over UDP (v1: full state every tick, not a diff -- see S8.4b bullet below)
- [x] Reliable channel for events (seq + ack-bitfield resend, no TCP -- see S8.4c bullet below; routing a real rule-trigger event over it is a follow-up, TODO.md)
- [x] Full state snapshot on player join
- [x] Client-side render interpolation between the last two synced positions, no prediction (see S8.5 bullet below)
